...
-- simple status line
-- vim.o.statusline = ' %F%m%r%h %w%=%{FugitiveStatusline()} l: %l c: %c '
...
vim.keymap.set('n', '<leader>t', ':e ~/todo.txt<cr>')
vim.keymap.set('v', '>', '>gv')
vim.keymap.set('v', '<', '<gv')
...
vim.keymap.set(
  'n',
  '<leader>au',
  'i// <esc>kopreHandler: (request, _reply, done) => { request.user = { sub: "72954474-f091-7008-ca5d-f77a91c8ae64" }; done() },<esc>'
)
